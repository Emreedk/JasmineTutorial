describe('main.js', function () {
  describe('calculate()', function () {
    ...
    it('Calls subtract', function () {
      ...
      calculate('3-2');
      expect(spy).toHaveBeenCalled();
      expect(spy).toHaveBeenCalledWith(2);   //bu method sadece ikinci sayıyı kullanıyor.
      expect(spy).not.toHaveBeenCalledWith(3);  //bu method birinci sayıyı kullanmıyor.
    ...
    it('calls updateResult (example for callThrough)', function () {
      spyOn(window, 'updateResult');
      spyOn(Calculator.prototype, 'multiply').and.callThrough();  //spyOn ile bu methodu çağırdığımızda aslında gerçek method çalışmıyor. yani dolayısıyla template'e herhangi bir değer gitmiyor. Gerçek methodu çağırıyor ve Bu değerin dönmesi için callTrough kullanılıyor.
      calculate('3*9');
